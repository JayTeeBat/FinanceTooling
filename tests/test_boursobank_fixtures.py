...
_FIXTURE_ROOT = Path(__file__).parent / "fixtures" / "boursobank"
_CASES_PATH = _FIXTURE_ROOT / "cases.json"


class _BoursobankFixtureCase(TypedDict):
    id: str
    description: str
    file_name: str
    text_file: str
...
def _load_cases() -> list[_BoursobankFixtureCase]:
    raw = json.loads(_CASES_PATH.read_text(encoding="utf-8"))
    return [cast(_BoursobankFixtureCase, case) for case in raw]


def test_boursobank_fixture_manifest_is_valid() -> None:
    cases = _load_cases()
    ids = [case["id"] for case in cases]
    assert len(ids) == len(set(ids))
    for case in cases:
        fixture_path = _FIXTURE_ROOT / case["text_file"]
        assert fixture_path.exists(), f"Missing Boursobank fixture text file: {fixture_path}"


@pytest.mark.parametrize("case", _load_cases(), ids=lambda case: case["id"])
def test_boursobank_parser_fixture_regression(case: _BoursobankFixtureCase) -> None:
