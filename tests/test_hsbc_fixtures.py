...
_FIXTURE_ROOT = Path(__file__).parent / "fixtures" / "hsbc"
_CASES_PATH = _FIXTURE_ROOT / "cases.json"


class _HsbcFixtureCase(TypedDict):
    id: str
    description: str
    file_name: str
    text_file: str
...
def _load_cases() -> list[_HsbcFixtureCase]:
    raw = json.loads(_CASES_PATH.read_text(encoding="utf-8"))
    return [cast(_HsbcFixtureCase, case) for case in raw]


def test_hsbc_fixture_manifest_is_valid() -> None:
    cases = _load_cases()
    ids = [case["id"] for case in cases]
    assert len(ids) == len(set(ids))
    for case in cases:
        fixture_path = _FIXTURE_ROOT / case["text_file"]
        assert fixture_path.exists(), f"Missing HSBC fixture text file: {fixture_path}"


@pytest.mark.parametrize("case", _load_cases(), ids=lambda case: case["id"])
def test_hsbc_parser_fixture_regression(case: _HsbcFixtureCase) -> None:
    parser = HsbcParser()
    fixture_path = _FIXTURE_ROOT / case["text_file"]
