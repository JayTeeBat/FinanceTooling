...
def test_migrate_canonical_dataframe_backfills_category_ids_and_labels() -> None:
    ...
    assert migrated.loc[0, "category_id"] == "shopping.apparel"
    assert migrated.loc[0, "reporting_category_id"] == "shopping.apparel"
    assert migrated.loc[0, "category"] == "Shopping"
    assert migrated.loc[0, "subcategory"] == "Apparel"
...
def test_migrate_override_store_rewrites_entries_to_category_id_only() -> None:
    ...
    assert entry.category_id == "shopping.apparel"
