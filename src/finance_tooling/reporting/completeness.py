...
def build_completeness_report_from_dataframe(
    source_files: list[Path],
    ...
    *,
    ...
    """Build completeness report from canonical transaction dataframe rows."""
    source_unique = sorted({Path(str(path)) for path in source_files}, key=lambda path: str(path))
    source_statement_unique = [
        path for path in source_unique if classify_statement_type(path) == "statement"
    ]
    ...
    if parsed_transactions.empty or "source_file" not in parsed_transactions.columns:
        parsed_source_unique: list[Path] = []
        parsed_transactions_by_bank: Counter[str] = Counter()
    else:
        parsed_source_unique = sorted(
            {
                Path(str(value))
                for value in parsed_transactions["source_file"].dropna().tolist()
                if str(value).strip()
            },
            key=lambda path: str(path),
        )
