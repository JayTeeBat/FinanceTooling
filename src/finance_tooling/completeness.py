...
def build_completeness_report(
    ...
    *,
    ...
    missing_files = [path for path in source_unique if path not in parsed_source_set]

    source_count = len(source_unique)
    parsed_unique_count = len(parsed_source_unique)
    coverage_ratio = (parsed_unique_count / source_count) if source_count else 1.0
    ...
    missing_by_year = Counter(guess_source_year(path) for path in missing_files)
    missing_by_bank_guess = Counter(guess_source_bank(path) for path in missing_files)

    reasons: list[str] = []
    if source_count == 0:
        status = "pass"
        reasons.append("No source PDFs discovered")
    ...
    if missing_files:
        reasons.append(f"{len(missing_files)} source PDFs have zero parsed rows")
    ...
    return {
        ...
        "missing_grouped_summaries": {
            ...
            "by_year_and_bank_guess": _group_missing_by_year_and_bank(missing_files),
        },
        "missing_source_files": [str(path) for path in missing_files],
    }
